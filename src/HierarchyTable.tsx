...
function EmployeeAvatar({ id, name }: { id: string; name: string }) {
  ...
  if (failed) {
    ...
    return <span className="employee-avatar-fallback grid size-6 flex-none place-items-center rounded-full bg-[#e6defd] text-[10px]/none font-semibold text-[#5c438b]" aria-hidden="true">{initials}</span>;
  }
  return <img className="employee-avatar block size-6 flex-none rounded-full object-cover" src={`/api/avatars/${id}.jpg`} alt="" width="24" height="24"
...
export function HierarchyTable({ root, selectedId, onSelect }: Props) {
  ...
  return <section ref={panel} className="min-w-0 max-w-full overflow-hidden rounded-lg bg-white" aria-label="Client breakdown" style={{ '--visible-months': monthCount } as React.CSSProperties}>
    ...
    <div className="w-full overflow-x-auto focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#795bd6] max-[1440px]:overflow-x-hidden" tabIndex={0} aria-label="Client breakdown table">
      <table className="w-full min-w-[1408px] table-fixed border-collapse text-sm leading-5 tabular-nums max-[1440px]:min-w-0 max-[1440px]:[--label-width:280px] max-[601px]:[--label-width:200px] max-[421px]:[--label-width:210px]" role="treegrid" aria-label="Client breakdown by month">
        <thead><tr><th className="sticky left-0 z-20 h-14 w-[280px] border-b border-ink/8 bg-white p-0 text-right font-normal whitespace-nowrap text-ink/60 max-[1440px]:static max-[601px]:w-[200px] max-[421px]:w-[210px]" scope="col"><span className="sr-only">Business unit</span></th>{months.map((month, index) => <th scope="col" key={month} className={`${monthClass(index >= firstMonth && index < firstMonth + monthCount)} h-14 w-[92px] border-b border-ink/8 p-0 pl-4 text-right font-normal whitespace-nowrap text-ink/60 last:w-[116px] last:pr-6 max-[1440px]:w-[calc((100%_-_var(--label-width))/var(--visible-months))] max-[1440px]:pl-2 max-[1440px]:pr-4 max-[1440px]:last:w-[calc((100%_-_var(--label-width))/var(--visible-months))] max-[1440px]:last:pr-4`}>{month.replace(' ', ' 20')}</th>)}</tr></thead>
        <tbody>{rows.map((row, index) => {
          ...
          return <tr className="group" key={row.node.id} aria-level={row.level + 1} aria-posinset={row.position} aria-setsize={row.siblingCount} aria-expanded={children.length ? isExpanded : undefined}>
            <th className="sticky left-0 z-10 h-[55px] w-[280px] border-b border-ink/8 bg-white p-0 text-left font-normal whitespace-nowrap group-hover:bg-[#faf9f5] max-[1440px]:static max-[601px]:w-[200px] max-[421px]:w-[210px]" scope="row"><div className="flex h-[55px] items-center gap-2 pr-2 ps-[calc(var(--level)*28px+16px)] max-[1440px]:min-w-0 max-[601px]:ps-[calc(var(--level)*14px+12px)]" style={{ '--level': row.level } as React.CSSProperties}>
              {children.length ? <button ref={element => { if (element) buttons.current.set(row.node.id, element); else buttons.current.delete(row.node.id); }} className="grid h-5 w-4 flex-none cursor-pointer place-items-center border-0 bg-transparent p-0 text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#795bd6]" type="button" aria-expanded={isExpanded} aria-label={`${isExpanded ? 'Collapse' : 'Expand'} ${row.node.name}`} onClick={() => toggle(row.node.id)} onKeyDown={event => onRowKeyDown(event, index)}><span className={`size-[6px] border-r-[1.5px] border-b-[1.5px] border-current transition-transform duration-150 ${isExpanded ? 'rotate-45' : '-rotate-45'}`} aria-hidden="true" /></button> : kind === 'branch' ? <span className="grid h-5 w-4 flex-none place-items-center text-ink" aria-hidden="true"><span className="size-[6px] -rotate-45 border-r-[1.5px] border-b-[1.5px] border-current" /></span> : <span className="w-4 flex-none" aria-hidden="true" />}
              {kind === 'employee' && <EmployeeAvatar id={row.node.id} name={row.node.name} />}
              <button ref={element => { if (!children.length) { if (element) buttons.current.set(row.node.id, element); else buttons.current.delete(row.node.id); } }} type="button" className="cursor-pointer border-0 bg-transparent p-0 text-left font-normal whitespace-nowrap text-ink hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#795bd6] max-[1440px]:min-w-0 max-[1440px]:overflow-hidden max-[1440px]:text-ellipsis" aria-current={isSelected ? 'true' : undefined} aria-label={`${row.node.name}, ${kind}, level ${row.level + 1}${row.parentId ? ', child row' : ''}; show in chart`} onClick={() => onSelect(row.node)} onKeyDown={event => onRowKeyDown(event, index)}>{row.node.name}</button>
            </div></th>
            {row.node.values.map((value, month) => <td key={month} className={`${monthClass(month >= firstMonth && month < firstMonth + monthCount)} h-[55px] w-[92px] border-b border-ink/8 bg-white p-0 pl-4 text-right text-ink group-hover:bg-[#faf9f5] last:w-[116px] last:pr-6 max-[1440px]:w-[calc((100%_-_var(--label-width))/var(--visible-months))] max-[1440px]:pl-2 max-[1440px]:pr-4 max-[1440px]:last:w-[calc((100%_-_var(--label-width))/var(--visible-months))] max-[1440px]:last:pr-4`}>{value.toLocaleString()}</td>)}
          </tr>;
        })}</tbody>
      </table>
    </div>
