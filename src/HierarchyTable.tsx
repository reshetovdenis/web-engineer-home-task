      ...
      <table className="w-full min-w-[1408px] table-fixed border-collapse text-sm leading-5 tabular-nums max-[1440px]:min-w-0 max-[1440px]:[--label-width:280px] max-[601px]:[--label-width:200px] max-[421px]:[--label-width:210px]" role="treegrid" aria-label="Client breakdown by month">
        ...
        <tbody>{rows.map((row, index) => {
          ...
          return <tr className="group" key={row.node.id} aria-level={row.level + 1} aria-posinset={row.position} aria-setsize={row.siblingCount} aria-expanded={children.length ? isExpanded : undefined}>
            <th className="sticky left-0 z-10 h-14 w-[280px] border-b border-ink/8 bg-white p-0 text-left font-normal whitespace-nowrap group-hover:bg-[#faf9f5] max-[1440px]:static max-[601px]:w-[200px] max-[421px]:w-[210px]" scope="row"><div className="flex h-14 items-center gap-2 pr-2 ps-[calc(var(--level)*28px+16px)] max-[1440px]:min-w-0 max-[601px]:ps-[calc(var(--level)*14px+12px)]" style={{ '--level': row.level } as React.CSSProperties}>
            ...
            {row.node.values.map((value, month) => <td key={month} className={`${monthClass(month >= firstMonth && month < firstMonth + monthCount)} h-14 w-[92px] border-b border-ink/8 bg-white p-0 pl-4 text-right text-ink group-hover:bg-[#faf9f5] last:w-[116px] last:pr-6 max-[1440px]:w-[calc((100%_-_var(--label-width))/var(--visible-months))] max-[1440px]:pl-2 max-[1440px]:pr-4 max-[1440px]:last:w-[calc((100%_-_var(--label-width))/var(--visible-months))] max-[1440px]:last:pr-4`}>{value.toLocaleString()}</td>)}
          </tr>;
        })}</tbody>
      </table>
    </div>
